 ...
 * - Deposits USDC into Yearn vault for yield
 ...
 * - Cross-chain position tracking via OVault/Yield Routes
 */
contract USDXVault is AccessControl, Pausable, ReentrancyGuard {
    ...
    /// @notice User Yearn shares mapping
    ...
    event Deposited(address indexed user, uint256 usdcAmount, uint256 usdxAmount, uint256 yearnShares);
    ...
    function deposit(uint256 amount) external nonReentrant whenNotPaused returns (uint256 usdxMinted) {
        ...
        // Deposit into Yearn vault if configured
        uint256 yearnShares = 0;
        if (address(yearnVault) != address(0)) {
            // Approve Yearn vault
            usdc.approve(address(yearnVault), amount);
            
            // Deposit into Yearn and get shares
            yearnShares = yearnVault.deposit(amount, address(this));
        ...
        emit Deposited(msg.sender, amount, usdxMinted, yearnShares);
        return usdxMinted;
    }
    ...
    /**
     * @notice Gets user's Yearn vault shares
     * @param user User address
    ...
    /**
     * @notice Gets current vault value (USDC + yield if Yearn is configured)
